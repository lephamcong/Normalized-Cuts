...
#Load image for test
path1 = r"Image_processing/Image-Segmentation/TestInputs/img1.jpg"
src1 = cv2.imread(path1)
src1 = resize(src1, (src1.shape[0] // 4, src1.shape[1] // 4),anti_aliasing=True)[:,:,::-1]
path2 = r"Image_processing/Image-Segmentation/TestInputs/rgb4.jpg"
...
def indRad(img,r) :
    ...
    ind[:,r:-r,r:-r] = np.indices(img[:,:,0].shape)
    indr = ind[0]
    indc = ind[1]
    ...
    indnorm = norm(np.array([diffr,diffc]),axis = 0)   #Incdices with norm within r

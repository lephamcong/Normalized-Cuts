...
def Ncut(thres,eigvecSec,D,W) :
    ...
    '''
    y = np.zeros(eigvecSec.shape)  #Divide into partitions based on thres
    athres = np.where(eigvecSec >= thres)[0]
    bthres = np.where(eigvecSec < thres)[0] #Define k as 'b' in paper
    k = (D.tocsr()[athres, athres].sum()) / (D.tocsr()[bthres, bthres].sum())
    y[athres] = 1
...
def gpart(D,W,e=2) :
    ...
    thres = scipy.optimize.fmin(func = Ncut,x0 = np.median(eigvecSec),args = (eigvecSec,D,W))[0]
...
def recpart(part,W,start,Arthres=1500,NCthres=0.03) :
    ...
    if start == True :
        ...
        col = np.array(list(part)*len(part))
...
def ImgSeg(img) :
    ...
    D = Dgraph(graph)               #Generate D
                                    #Generate partition
    partition,status = recpart(np.arange(len(img.flatten())),graph,True)
                                    #Output shape to hold all partitions concatanated
    imgSeg = np.zeros((img.shape[0],int((len(partition)+1)*img.shape[1])))                    
    imgSeg[:,:img.shape[1]] = img  #First section is the original image
    for i in range(0,len(partition)) :
        coloffset = img.shape[1]*(i+1)
        athres = partition[i]
                                    #Generate row col indices from W indices
        athresc = athres% img.shape[1]
        athresr = (athres-athresc)//img.shape[1]

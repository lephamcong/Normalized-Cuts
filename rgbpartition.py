...
def ImgSeg(img) :
    ...
    graph = wgraph(img)
    D = Dgraph(graph)
    partition,status = recpart(np.arange(len(img[:,:,0].flatten())),graph,True)
    imgSeg = np.zeros((img.shape[0],(len(partition)+1)*img.shape[1],img.shape[2]))
    imgSeg[:,:img.shape[1],:] = img
